...
public class RealDebridTorrentClient : ITorrentClient
{
    ...
    public async Task SelectFiles(Data.Models.Data.Torrent torrent)
    {
        ...
        {
            TorrentDownloadAction.DownloadAvailableFiles => await FilterAvailableFiles(torrent, files),
    ...
    private async Task<List<TorrentClientFile>> FilterAvailableFiles(Data.Models.Data.Torrent torrent, List<TorrentClientFile> files)
    {
    ...
    {
        if (torrent.RdId == null) return null;

        var rdTorrent = await GetInfo(torrent.RdId);

        if (rdTorrent.Links == null) return null;
    ...
    {
        var result = await (await GetClientAsync()).Torrents.GetInfoAsync(torrentId);
        return Map(result);
    }
